...
ZEROIFNULL(sum(net_new_arr) over (partition by fy_year order by dte asc rows between unbounded preceding and current row)) as net_new_arr,
ZEROIFNULL(sum(total_arr) over (partition by fy_year order by dte asc rows between unbounded preceding and current row)) as total_arr,
ZEROIFNULL(sum(num_opps) over (partition by fy_year order by dte asc rows between unbounded preceding and current row)) as num_opps
from fy_agg_int 
