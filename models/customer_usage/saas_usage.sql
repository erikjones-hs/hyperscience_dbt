...
qualify count(distinct period_start) over (partition by split_part(report_id,'_',0), report_id) = 1 
...
raw_data_dup as (
select * from raw_data_dup_int
where datediff(day,to_date(split_part(report_id,'_',2)),to_date(period_start)) = -1
qualify row_number() over (partition by customer order by period_start) >= 2
),
...
fct_saas_usage as (
select * from raw_data_non_dup
UNION 
select * from raw_data_dup
UNION 
