...
na_churn_arr,
na_churn_arr - lag(na_churn_arr,1,0) over(order by date_month asc) as na_churn_arr_yoy_change,
(na_churn_arr - lag(na_churn_arr,1,0) over(order by date_month asc)) / NULLIFZERO(lag(na_churn_arr,1,0) over(order by date_month asc)) as na_churn_arr_yoy_change_perc,                                         
emea_churn_arr,
emea_churn_arr - lag(emea_churn_arr,1,0) over(order by date_month asc) as emea_churn_arr_yoy_change,
(emea_churn_arr - lag(emea_churn_arr,1,0) over(order by date_month asc)) / NULLIFZERO(lag(emea_churn_arr,1,0) over(order by date_month asc))  as emea_churn_arr_yoy_change_perc,                                         
apac_churn_arr,
apac_churn_arr - lag(apac_churn_arr,1,0) over(order by date_month asc) as apac_churn_arr_yoy_change,
(apac_churn_arr - lag(apac_churn_arr,1,0) over(order by date_month asc)) / NULLIFZERO(lag(apac_churn_arr,1,0) over(order by date_month asc))  as apac_churn_arr_yoy_change_perc,                                        
de_book_arr,
