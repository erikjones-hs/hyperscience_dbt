...
select 
    ...
    opportunity_id,
    max(case when new_value = 'Value/Fit' then 1 when new_value = 'Discovery & Qualification' then 1 else 0 end) as had_value_fit,
    max(case when new_value = 'TDD' then 1 when new_value = 'Alignment' then 1 else 0 end) as had_tdd,
