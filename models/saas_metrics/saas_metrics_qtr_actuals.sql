...
to_decimal(healthcare_benefits,38,2) as healthcare_benefits,
to_decimal(referral_bonus,38,2) as referral_bonus,
to_decimal(commissions,38,2) as commissions,
to_decimal(guarantees,38,2) as guarantees,
to_decimal(cash_balance,38,2) as cash_balance,
...
from "FIVETRAN_DATABASE"."GOOGLE_SHEETS"."FINANCE_ACTUALS"
order by date asc
),

actuals_qtr_transform as (
select distinct
date,
...
(salary_wage_allocation + payroll_tax_allocation + benefits_allocation + exmployer_401_k_allocation + 
 ...
 bonus + severance + relocation + temp_labor + healthcare_benefits + referral_bonus + commissions + guarantees) as comp_benefits_numerator,
CASE WHEN to_date(date) in ('2023-02-28','2023-05-31','2023-08-31','2023-11-30','2024-02-29','2024-05-31','2024-08-31','2024-11-30','2025-02-28') then cash_balance else 0 end as cash_balance,
...
AWS_EXPENSE,
SOFTWARE_PER_FTE_NUMERATOR,
to_decimal(software_per_fte_numerator,38,2) as software_per_fte,
REAL_ESTATE_EXPENSE,
R_AND_D_EXPENSE,
S_AND_M_EXPENSE,
G_AND_A_EXPENSE,
...
to_decimal(comp_benefits_numerator,38,2) as comp_and_benefits_per_rev,
cash_balance,
ENG_FTE,
SALES_FTE,
CX_FTE,
...
LEGAL_FTE,
ALL_DEPT_FTE
from actuals_qtr_aggregate_int
order by qtr_end_dte asc
),

actuals_qtr_pivot as (
select * from actuals_qtr_aggregate
  unpivot(actuals for metric in (ARR, REVENUE, NET_DOLLAR_RETENTION, GROSS_DOLLAR_RETENTION, NET_LOGO_RETENTION, GROSS_MARGIN, GROSS_MARGIN_PERCENT,	
                                CAC_PAYBACK_PERIOD,	CASH_CONVERSION_SCORE, RULE_OF_40, MAGIC_NUMBER, LTV_TO_CAC, FCF_MARGIN_NUMERATOR, fcf_margin,	
                                NET_NEW_ARR, NEW_ARR, CHURN_ARR, BURN_MULTIPLE_NUMERATOR, burn_multiple, AWS_EXPENSE, SOFTWARE_PER_FTE_NUMERATOR, software_per_fte,	
                                ...
                                cash_balance, ENG_FTE, SALES_FTE, CX_FTE, MARKETING_FTE, PRODUCT_FTE, FINANCE_FTE, PEOPLE_FTE, LEGAL_FTE, ALL_DEPT_FTE))
order by qtr_end_dte asc
)

select * from actuals_qtr_pivot