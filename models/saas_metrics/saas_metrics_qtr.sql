...
CASE WHEN b.metric = 'ARR' then 1
     ...
     when b.metric = 'BURN_MULTIPLE_NUMERATOR' then 14
     when b.metric = 'NET_NEW_ARR' then 15
     when b.metric = 'BURN_MULTIPLE' then 16
     when b.metric = 'AWS_EXPENSE' then 17
     when b.metric = 'SOFTWARE_PER_FTE' then 18
     when b.metric = 'REAL_ESTATE_EXPENSE' then 19
     when b.metric = 'R_AND_D_EXPENSE' then 20
     when b.metric = 'S_AND_M_EXPENSE' then 21
     when b.metric = 'G_AND_A_EXPENSE' then 22
     when b.metric = 'TOTAL_OPEX' then 23
     when b.metric = 'GAAP_REV_PER_FTE' then 24
     when b.metric = 'COMP_AND_BENEFITS_PER_REV' then 25
     when b.metric = 'ENG_FTE' then 26
     when b.metric = 'SALES_FTE' then 27
     when b.metric = 'CX_FTE' then 28
     when b.metric = 'MARKETING_FTE' then 29
     when b.metric = 'PRODUCT_FTE' then 30
...
CASE WHEN b.metric = 'ARR' then 'ARR'
     ...
     when b.metric = 'BURN_MULTIPLE_NUMERATOR' then 'Cash Burn'
     ...
     when b.metric = 'CASH_BALANCE' then 'Cash Balance'
