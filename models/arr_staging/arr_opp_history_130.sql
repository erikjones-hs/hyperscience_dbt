...
raw_data_inc_106 as (
select * from {{ref('arr_opp_history_107')}}    
),

raw_data_inc_107 as (
select * from {{ref('arr_opp_history_108')}}    
),

raw_data_inc_108 as (
select * from {{ref('arr_opp_history_109')}}    
),

raw_data_inc_109 as (
select * from {{ref('arr_opp_history_110')}}    
),

raw_data_inc_110 as (
select * from {{ref('arr_opp_history_111')}}    
),

raw_data_inc_111 as (
select * from {{ref('arr_opp_history_112')}}    
),

raw_data_inc_112 as (
select * from {{ref('arr_opp_history_113')}}    
),

raw_data_inc_113 as (
select * from {{ref('arr_opp_history_114')}}    
),

raw_data_inc_114 as (
select * from {{ref('arr_opp_history_115')}}    
),

raw_data_inc_115 as (
select * from {{ref('arr_opp_history_116')}}    
),

raw_data_inc_116 as (
select * from {{ref('arr_opp_history_117')}}    
),

raw_data_inc_117 as (
select * from {{ref('arr_opp_history_118')}}    
),

raw_data_inc_118 as (
select * from {{ref('arr_opp_history_119')}}    
),

raw_data_inc_119 as (
select * from {{ref('arr_opp_history_120')}}    
),

raw_data_inc_120 as (
select * from {{ref('arr_opp_history_121')}}    
),

raw_data_inc_121 as (
select * from {{ref('arr_opp_history_122')}}    
),

raw_data_inc_122 as (
select * from {{ref('arr_opp_history_123')}}    
),

raw_data_inc_123 as (
select * from {{ref('arr_opp_history_124')}}    
),
...
raw_data_inc_127 as (
select * from {{ref('arr_opp_history_128')}}    
),

raw_data_inc_128 as (
select * from {{ref('arr_opp_history_129')}}    
),
...
opp_arr,
opp_net_new_arr
from "DEV"."ERIKJONES"."SALESFORCE_AGG_OPPORTUNITY_v129"
...
and opp_id not in (select opp_id from raw_data_inc_102)
and opp_id not in (select opp_id from raw_data_inc_103)
and opp_id not in (select opp_id from raw_data_inc_104)
and opp_id not in (select opp_id from raw_data_inc_105)
and opp_id not in (select opp_id from raw_data_inc_106)
and opp_id not in (select opp_id from raw_data_inc_107)
and opp_id not in (select opp_id from raw_data_inc_108)
and opp_id not in (select opp_id from raw_data_inc_109)
and opp_id not in (select opp_id from raw_data_inc_110)
and opp_id not in (select opp_id from raw_data_inc_111)
and opp_id not in (select opp_id from raw_data_inc_112)
and opp_id not in (select opp_id from raw_data_inc_113)
and opp_id not in (select opp_id from raw_data_inc_114)
and opp_id not in (select opp_id from raw_data_inc_115)
and opp_id not in (select opp_id from raw_data_inc_116)
and opp_id not in (select opp_id from raw_data_inc_117)
and opp_id not in (select opp_id from raw_data_inc_118)
and opp_id not in (select opp_id from raw_data_inc_119)
and opp_id not in (select opp_id from raw_data_inc_120)
and opp_id not in (select opp_id from raw_data_inc_121)
and opp_id not in (select opp_id from raw_data_inc_122)
and opp_id not in (select opp_id from raw_data_inc_123)
and opp_id not in (select opp_id from raw_data_inc_124)
and opp_id not in (select opp_id from raw_data_inc_125)
and opp_id not in (select opp_id from raw_data_inc_126)
and opp_id not in (select opp_id from raw_data_inc_127)
and opp_id not in (select opp_id from raw_data_inc_128)
and opp_arr > 0
and opp_id not in ('006Pm00000CXKYvIAP','0061R000016nuiyQAA') /* Removing a test opportunity */
order by account_id, start_dte_raw asc
)

select * from raw_data_inc_129